...
Dùng để xuất nhiều biến, hằng số hoặc hàm từ cùng một file. Khi import, bạn phải dùng chính xác tên đó đặt trong cặp ngoặc nhọn `{}`.
...
Mỗi file chỉ được phép có duy nhất **một** default export. Thường dùng khi file đó chỉ đại diện cho một class, một component hoặc một tập hợp hàm chính. Khi import, bạn có thể đặt tên tùy ý và không dùng ngoặc nhọn `{}`.
